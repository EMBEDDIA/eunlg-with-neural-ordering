...
class EUFilterSetpenCtxBodyDocumentPlanner(BodyDocumentPlanner):
...
class EUFilterSetpenCtxHeadlineDocumentPlanner(HeadlineDocumentPlanner):
    def select_next_nucleus(
        self, available_message: List[Message], selected_nuclei: List[Message]
    ) -> Tuple[Message, float]:
        return _select_next_nucleus(available_message, selected_nuclei)
...
def _select_satellites_for_nucleus(
    ...
    while True:
        ...
        scores_v_nucleus = _weigh_by_context_similarity(scored_available_messages, nucleus)
        scores_v_nucleus = {message: score for (score, message) in scores_v_nucleus}
        scores_v_prev = _weigh_by_context_similarity(scored_available_messages, previous)
