...
class EUMessageGenerator(MessageGenerator):
    """
    An NLGPipelineComponent that creates messages from StatFi crime statistics splits.
    """
    ...
    def run(
        ...
        log.info(
            "Generating messages with location={}, location_type={}, splits={}".format(
                location_query, location_type_query, dataset,
            )
        )

        data_store: DataFrameStore = registry.get("{}-splits".format(dataset))
    ...
    def _gen_messages(
        ...
        for col_name in col_names:
            ...
            # grouping the splits. TODO: Smarter way to select which on the use
            if not outlierness:
