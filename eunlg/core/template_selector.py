...
class TemplateSelector(NLGPipelineComponent):
    ...
    def _value_type_is_substantially_similar(self, first: Message, second: Message) -> bool:
        ...
        # ignore whether this is a rank of the raw value
        if ":rank" in first_type:
            first_type = first_type[: first_type.find(":rank")]
        if ":rank" in second_type:
            second_type = second_type[: second_type.find("rank")]

        # ignore whether this is a comparison of the raw value
